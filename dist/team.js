...
${cause.stack}`;
  ...
  function toTransportFailure(cause, status2, statusText) {
    var _err$name;
    const err = cause;
    return {
      success: false,
...
Suggested solution: ${env.workaround}`;
    ...
    */
    async createSignedUrl(path, expiresIn, options) {
      ...
      return _this8.handleOperation(async () => {
        ...
        let data2 = await post(_this8.fetch, `${_this8.url}/object/sign/${_path}`, _objectSpread22(_objectSpread22({ expiresIn }, hasTransform ? { transform: options.transform } : {}), (options === null || options === void 0 ? void 0 : options.versionId) != null ? { versionId: options.versionId } : {}), { headers: _this8.headers });
  ...
  async function handleError2(error) {
    ...
    if (NETWORK_ERROR_CODES.includes(error.status)) {
      throw new AuthRetryableFetchError(_getErrorMessage2(data2), error.status);
    }
  ...
  function _sessionResponse(data2) {
    ...
    let session2 = null;
    if (hasSession(data2)) {
      session2 = Object.assign({}, data2);
      if (!data2.expires_at) {
        session2.expires_at = expiresAt(data2.expires_in);
      }
    }
  ...
  var GoTrueAdminApi = class {
    ...
    async _adminListPasskeys(params) {
      ...
      validateUUID(params.userId);
      try {
        return await _request(this.fetch, "GET", `${this.url}/admin/users/${params.userId}/passkeys`, { headers: this.headers, xform: (data2) => ({ data: data2, error: null }) });
      } catch (error) {
        if (isAuthError(error)) {
          return { data: null, error };
        }
...
${suffix}`;
  ...
  var GoTrueClient = class _GoTrueClient {
    ...
    async _initialize() {
      ...
      try {
        ...
        if (isBrowser() && this.detectSessionInUrl && callbackUrlType !== "none") {
          ...
          const { session: session2, redirectType } = data2;
    ...
    async _notifyAllSubscribers(event, session2, broadcast = true) {
      if (this._pendingInitNotifications !== null && broadcast) {
        this._pendingInitNotifications.push({ event, session: session2, broadcast });
        return;
      }
        ...
        try {
          return await this._useSession(async (result) => {
            ...
            switch (data2.webauthn.type) {
              case "create":
                return {
                  data: Object.assign(Object.assign({}, data2), { webauthn: Object.assign(Object.assign({}, data2.webauthn), { credential_options: Object.assign(Object.assign({}, data2.webauthn.credential_options), { publicKey: deserializeCredentialCreationOptions(data2.webauthn.credential_options.publicKey) }) }) }),
                  error: null
                };
              case "request":
                return {
                  data: Object.assign(Object.assign({}, data2), { webauthn: Object.assign(Object.assign({}, data2.webauthn), { credential_options: Object.assign(Object.assign({}, data2.webauthn.credential_options), { publicKey: deserializeCredentialRequestOptions(data2.webauthn.credential_options.publicKey) }) }) }),
                  error: null
                };
            }
          });
    ...
    async _listFactors() {
      ...
      for (const factor of (_a = user === null || user === void 0 ? void 0 : user.factors) !== null && _a !== void 0 ? _a : []) {
        ...
        if (factor.status === "verified" && factor.factor_type in data2 && Array.isArray(data2[factor.factor_type])) {
          ;
    ...
    async _getRecoveryCodesStatus() {
      ...
      try {
        return await this._useSession(async (result) => {
          ...
          const { data: data2, error } = await _request(this.fetch, "GET", `${this.url}/factors/recovery-codes`, {
            headers: this.headers,
            jwt: (_a = sessionData === null || sessionData === void 0 ? void 0 : sessionData.session) === null || _a === void 0 ? void 0 : _a.access_token
          });
          if (error) {
    ...
    async _generateRecoveryCodes(params) {
      ...
      try {
        return await this._useSession(async (result) => {
          ...
          const { data: data2, error } = await _request(this.fetch, "POST", `${this.url}/factors/recovery-codes`, {
      ...
      const run = async () => {
        try {
          return await this._useSession(async (result) => {
            ...
            const { data: data2, error } = await _request(this.fetch, "POST", `${this.url}/factors/recovery-codes/verify`, {
            ...
            const session2 = Object.assign({ expires_at: expiresAt(data2.expires_in) }, data2);
            await this._saveSession(session2);
            await this._notifyAllSubscribers("MFA_CHALLENGE_VERIFIED", session2);
            return this._returnResult({ data: data2, error: null });
          });
        } catch (error) {
          if (isAuthError(error)) {
    ...
    async _regenerateRecoveryCodes() {
      ...
      try {
        return await this._useSession(async (result) => {
          ...
          const { data: data2, error } = await _request(this.fetch, "POST", `${this.url}/factors/recovery-codes/regenerate`, {
            headers: this.headers,
            jwt: (_a = sessionData === null || sessionData === void 0 ? void 0 : sessionData.session) === null || _a === void 0 ? void 0 : _a.access_token
          });
          if (error) {
    ...
    async _unenrollRecoveryCodes() {
      ...
      try {
        return await this._useSession(async (result) => {
          ...
          const { data: data2, error } = await _request(this.fetch, "DELETE", `${this.url}/factors/recovery-codes`, {
            headers: this.headers,
            jwt: (_a = sessionData === null || sessionData === void 0 ? void 0 : sessionData.session) === null || _a === void 0 ? void 0 : _a.access_token
          });
          if (error) {
    ...
    async _startPasskeyRegistration() {
      ...
      try {
        return await this._useSession(async (result) => {
          ...
          const { data: data2, error } = await _request(this.fetch, "POST", `${this.url}/passkeys/registration/options`, {
            headers: this.headers,
            jwt: session2.access_token,
            body: {}
          });
    ...
    async _verifyPasskeyRegistration(params) {
      ...
      try {
        return await this._useSession(async (result) => {
          ...
          const { data: data2, error } = await _request(this.fetch, "POST", `${this.url}/passkeys/registration/verify`, {
            headers: this.headers,
            jwt: session2.access_token,
            body: {
    ...
    async _startPasskeyAuthentication(params) {
      ...
      assertPasskeyExperimentalEnabled(this.experimental);
      try {
        const { data: data2, error } = await _request(this.fetch, "POST", `${this.url}/passkeys/authentication/options`, {
          headers: this.headers,
          body: {
    ...
    async _verifyPasskeyAuthentication(params) {
      assertPasskeyExperimentalEnabled(this.experimental);
      try {
        const { data: data2, error } = await _request(this.fetch, "POST", `${this.url}/passkeys/authentication/verify`, {
          headers: this.headers,
          body: {
            challenge_id: params.challengeId,
            credential: params.credential
    ...
    async _listPasskeys() {
      ...
      try {
        return await this._useSession(async (result) => {
          ...
          const { data: data2, error } = await _request(this.fetch, "GET", `${this.url}/passkeys`, {
            headers: this.headers,
            jwt: session2.access_token,
    ...
    async _updatePasskey(params) {
      ...
      try {
        return await this._useSession(async (result) => {
          ...
          const { data: data2, error } = await _request(this.fetch, "PATCH", `${this.url}/passkeys/${params.passkeyId}`, {
            headers: this.headers,
            jwt: session2.access_token,
  ...
  var SupabaseClient = class {
    ...
    */
    ...
    _initSupabaseAuthClient({ autoRefreshToken, persistSession, detectSessionInUrl, storage, userStorage, storageKey, flowType, lock: lock2, debug, throwOnError, experimental, lockAcquireTimeout, skipAutoInitialize }, headers, fetch$1) {
      const authHeaders = {
  ...
  // src/team.js
  var config = window.EVIDENCEFLOW_CONFIG || {};
  var configured = Boolean(config.supabaseUrl && config.supabaseKey);
  var client = configured ? createClient(config.supabaseUrl, config.supabaseKey) : null;
  var session = null;
  var workspace = null;
  var revision = 0;
  var busy = false;
  ...
  status.innerHTML = '<button class="btn" id="team-account">Team account</button><select id="team-workspace" aria-label="Shared workspace" hidden></select><button class="btn secondary" id="team-refresh" hidden>Refresh shared data</button><span id="team-state" role="status">Personal workspace</span>';
  ...
  var message = () => dialog.querySelector("[role=status]");
  ...
  function lock(value) {
    document.getElementById("form").querySelectorAll("button").forEach((b) => b.disabled = value);
  }
  function restorePersonal() {
    workspace = null;
    window.EvidenceFlowCloud.active = false;
    data = JSON.parse(localStorage.getItem("evidenceflow-data") || "null") || structuredClone(seed);
    data.tests ||= [];
    document.querySelector(".local-notice").textContent = "Personal workspace \xB7 Records and files are stored in this browser.";
    document.querySelector(".workspace").firstChild.textContent = "Personal workspace ";
    document.getElementById("team-refresh").hidden = true;
    render();
    setState("Personal workspace");
  }
  async function loadWorkspace(id) {
    if (busy || document.getElementById("modal").classList.contains("open")) throw Error("Finish or cancel the open record before switching or refreshing.");
    const row = check(await client.from("ef_workspaces").select("*").eq("id", id).single());
    workspace = row;
    revision = row.revision;
    data = structuredClone(row.document);
    data.tests ||= [];
    window.EvidenceFlowCloud.active = true;
    for (const collection of [data.evidence, data.actions, data.tests]) for (const record of collection) if (record.period && ![...periodSelect.options].some((o) => o.value === record.period)) periodSelect.add(new Option(record.period, record.period));
    document.querySelector(".local-notice").textContent = "Shared workspace \xB7 Changes are saved to the team database. Use Refresh shared data to load teammates\u2019 latest changes.";
    document.querySelector(".workspace").firstChild.textContent = row.name + " ";
    document.getElementById("team-refresh").hidden = false;
    render();
    setState("Shared \xB7 revision " + revision);
  }
  window.EvidenceFlowCloud = { active: false, async save(document2) {
    ...
    setState("Saving\u2026");
    try {
      revision = check(await client.rpc("ef_save_workspace", { w: workspace.id, expected_revision: revision, new_document: document2 }));
      setState("Saved to team \xB7 revision " + revision);
      return true;
    } catch (error) {
      setState("Not saved \xB7 " + errorText(error));
      throw error;
    } finally {
      busy = false;
      lock(false);
  ...
  attachment = async function(id, file) {
    ...
    if (file) {
      ...
      check(await client.storage.from("evidenceflow").upload(path, file, { upsert: false, contentType: file.type || "application/octet-stream" }));
      return path;
    }
    const record = data.evidence.find((x) => String(x.id) === String(id));
    if (!record?.remotePath) throw Error("Attach this file to the shared record first.");
    const blob = check(await client.storage.from("evidenceflow").download(record.remotePath));
    return new File([blob], record.fileName || "evidence", { type: blob.type });
  };
  function accountMarkup() {
    if (!configured) return '<h2>Team setup pending</h2><p>The database connection has not been configured yet.</p><button type="button" data-close class="btn secondary">Close</button>';
    if (!session) return '<h2>Team account</h2><form id="auth-form"><div class="field"><label for="account-email">Email</label><input class="input" id="account-email" name="email" type="email" autocomplete="email" required></div><div class="field"><label for="account-password">Password</label><input class="input" id="account-password" name="password" type="password" minlength="8" autocomplete="current-password" required></div><div class="toolbar"><button class="btn" name="operation" value="signin">Sign in</button><button class="btn secondary" name="operation" value="signup">Create account</button></div></form><p role="status"></p><button type="button" data-close class="text-btn">Close</button>';
    return "<h2>Team workspace</h2><p>" + esc(session.user.email) + '</p><form id="create-workspace"><div class="field"><label for="workspace-name">New workspace name</label><input class="input" id="workspace-name" required maxlength="120"></div><button class="btn">Create workspace</button></form>' + (workspace?.owner_id === session.user.id ? '<form id="invite-member"><div class="field"><label for="invite-email">Teammate email</label><input class="input" id="invite-email" type="email" required></div><button class="btn secondary">Grant workspace invitation</button><p>Ask the teammate to create an account with this email, confirm it, and sign in here. This action does not send an email.</p></form>' : "") + '<p role="status"></p><div class="toolbar"><button type="button" class="btn secondary" id="signout">Sign out</button><button type="button" class="btn secondary" data-close>Close</button></div>';
  }
  function showAccount() {
    ...
    dialog.querySelector("#auth-form")?.addEventListener("submit", async (event) => {
      ...
      try {
        if (event.submitter.value === "signup") {
          const result = check(await client.auth.signUp({ ...credentials, options: { emailRedirectTo: location.origin + location.pathname } }));
          message().textContent = result.session ? "Account created." : "Check your email to confirm your account, then sign in.";
          if (result.session) await initializeSession(result.session);
        } else {
          const result = check(await client.auth.signInWithPassword(credentials));
    ...
    dialog.querySelector("#create-workspace")?.addEventListener("submit", async (event) => {
      event.preventDefault();
      try {
        const id = check(await client.rpc("ef_create_workspace", { workspace_name: dialog.querySelector("#workspace-name").value.trim() }));
    ...
    dialog.querySelector("#invite-member")?.addEventListener("submit", async (event) => {
      event.preventDefault();
      try {
        check(await client.rpc("ef_invite_member", { w: workspace.id, member_email: dialog.querySelector("#invite-email").value.trim() }));
        message().textContent = "Invitation granted. The teammate can join after confirming this email and signing in.";
      } catch (error) {
        message().textContent = errorText(error);
      }
    });
  ...
  async function listWorkspaces(prefer) {
    const rows = check(await client.from("ef_workspaces").select("id,name").order("name"));
    ...
    if (rows.length) {
      ...
      restorePersonal();
      setState("Signed in \xB7 create a workspace");
    }
  }
