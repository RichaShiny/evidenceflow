...
document.getElementById('form').onsubmit=async function(event){event.preventDefault();for(const input of this.querySelectorAll('input[required],textarea[required]')){if(!input.value.trim()){input.setCustomValidity('Enter a value.');input.reportValidity();input.oninput=()=>input.setCustomValidity('');return}}if(mode==='test'){const values=Object.fromEntries(new FormData(this));data.tests.unshift({id:crypto.randomUUID(),...values,control:selected,period:period()});try{await save()}catch{data.tests.shift();return}closeDialog();render();toast('Test saved with reviewer and conclusion');return}originalSubmit(event);returnFocus?.focus()};
...
document.getElementById('form').onsubmit=async function(event){event.preventDefault();if(mode==='test'){await workflowSubmit.call(this,event);return}if(!this.reportValidity())return;const values=Object.fromEntries(new FormData(this));for(const key of ['name','title','source','owner'])if(key in values&&!values[key].trim()){toast('Required fields cannot be blank.');return}
...
try{if(file?.size){const remotePath=await attachment(record.id,file);if(typeof remotePath==='string')record.remotePath=remotePath;record.fileName=file.name}if(index>=0)data[collection][index]=record;else data[collection].unshift(record);try{await save()}catch(error){if(index>=0)data[collection][index]=old;else data[collection].shift();throw error}closeDialog();render();toast('Record saved')}catch(error){toast('Could not save this record: '+error.message)}finally{button.disabled=false}};
